...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        if self.exit is not None:
            exit.entrance = self
        # place1's exit is place0
        # place0(place1's exit)'s entrance is "self" or place1
        # END Problem 2
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                #if ant can contain an insect passed to add_insect, then contain it
                if self.ant.is_container == True and self.ant.can_contain(insect) == True:
                    self.ant.contain_ant(insect)
                #if insect can contain an ant in the place, then contain it
                # set ant to the container insect
                elif insect.is_container == True and insect.can_contain(self.ant) == True:
                    insect.contain_ant(self.ant)
                    self.ant = insect # the ant that is currently at the place will be contained
                                        # by Insect so self.ant = Insect
                #if neither can contain it
                else:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if type(self.ant) == QueenAnt:
                if self.ant.the_Queen == True:
                #don't remove from place, do nothing
                    self.ant.place = self.ant.place
                    return
            if self.ant.is_container == True and type(self.ant.contained_ant) == QueenAnt:
                if self.ant.contained_ant.the_Queen == True:
                    self.ant.contained_ant == self.ant.contained_ant
                    return 
            # END Problem 13
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        no_ants = self.place.ant
        if no_ants == None or no_ants.blocks_path == False:
            return False # if not blocked
        return no_ants is not None #if there are ants that can block
        # END Problem 7
...
class ShortThrower(ThrowerAnt):
    ...
    # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem 4
    # cannot throw further than 3 places
    food_cost = 2
    armor = 1
    max_range = 3
...
class LongThrower(ThrowerAnt):
    ...
    # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem 4
    #cannot throw the first 4 places
    food_cost = 2
    armor = 1
    min_range = 5
    implemented = True   # Change to True to view in the GUI
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        #test out before mutating
        bees_left = [] #bees that survive
        bees_list = self.place.bees[:] #need to copy list
        #self.armor -= amount
        for b in bees_list:
            b.armor -= amount #reflective damage
            if self.armor - amount <= 0: #if ant dies
                b.armor -= self.damage

        for b in bees_list: #after changes to bees armor
            if b.armor <= 0: # if bee dies after amount or amount+damage
                b.death_callback()
            if b.armor > 0: #if bee survives
                bees_left.append(b)
        self.place.bees = bees_left
        Insect.reduce_armor(self,amount) #mutate
...
class HungryAnt(Ant):
    ...
    def __init__(self, armor=1): #add digesting counter here
        # BEGIN Problem 6
        Ant.__init__(self, armor)
        self.digesting = 0
        # END Problem 6
...
class BodyguardAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.contained_ant is not None:
            self.contained_ant.action(colony) #perform action of contained_ant
        # END Problem 9
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        for b in bees_list:
            b.reduce_armor(damage)
        #contained_ant execute its actions
        if self.contained_ant is not None:
...
# END Problem 13
    ...
    #need a way to track imposters
    def __init__(self, armor=1):
        # BEGIN Problem 13
        self.the_Queen = QueenAnt.the_Queen
        Ant.__init__(self, armor)
        QueenAnt.the_Queen = QueenAnt.not_imposter
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        #be able to locate imposter queens

        if self.the_Queen != True:
            self.reduce_armor(self.armor)
        #be able to do damage to bees infront of it
        else: #if true queen, not imposter
            place = self.place
            behind_Queen = place.exit
            while behind_Queen != None: #need to consider all places behind the queen until it hits the end

                if behind_Queen.ant != None:
                    if behind_Queen.ant not in self.doubled_ants:
                        behind_Queen.ant.damage *= 2
                        self.doubled_ants.append(behind_Queen.ant)
                if hasattr(behind_Queen.ant, "is_container") and behind_Queen.ant.is_container == True: #Bodyguard
                    contained_ant = behind_Queen.ant.contained_ant
            ...
            ThrowerAnt.action(self, colony) #need to attack not only double
        # END Problem 13
